    ...
    keyExperience: [
      {
        ...
        detail: {
          ...
          action: [
            "**1. Kafka 기반 비동기 처리 설계**: Kafka 설정은 파티션 15개, 복제 3로 설정했습니다. 이는 워커의 최대 스케일링 수(8개)에 맞춘 수치로, 안정적이고 효율적이도록 설계했습니다.",
            ...
            "**3. 백프레셔 기반 안정성 확보**: 워커가 Kafka 메시지를 처리 시 세마포어로 동시 처리 작업 수를 제한하여 워커 병목 방지 및 과부하로 인한 장애를 예방했습니다",
            "**4. CDN 기반 글로벌 이미지 서빙 최적화**: Akamai CDN을 도입하여 각 리전에서 생성된 이미지를 글로벌 사용자에게 가장 가까운 Endpoint에서 서빙하여 글로벌 사용자 경험을 최적화했습니다",
            "**5. 외부 API 장애 전파 대응 (Retry & Circuit Breaker)**: 이미지 생성 및 LLM 호출 실패 시 최대 3회 재시도 (초기 200ms 간격, 지수 백오프), 연속 10회 실패 시 Circuit Breaker open (30초 후 half-open 상태로 회복 시도)했습니다",
            "**6. 비용 관리 및 서비스 품질 모니터링**: 각 API 플랫폼당 월 최대 $4,000 예산 설정 (소진 75% 시 알람), 비용 및 서비스 품질을 모니터링할 수 있는 **내부 어드민 페이지를 구축**했습니다. 사용자의 이용 형태를 데이터 기반으로 확인 및 개선했습니다",
          ],
          result: [
            "개발 기간 4주만에 신규 런칭",
            "**uptime 99.996% 유지**",
