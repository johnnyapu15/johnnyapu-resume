    ...
    experience: [
      {
        ...
        period: "2022년 8월 - 현재",
        description: [
          "**Ohouse AI 런칭**: 6주 목표를 **4주로 앞당겨** **170개국** 글로벌 런칭을 완료했습니다. 사용자 요청 API와 장시간 AI 작업을 분리하도록 메시지 큐 기반 워커와 오토스케일링, 실패 격리, 상태 관리·재시도 패턴을 설계해 예측 불가 트래픽을 안정적으로 처리했습니다.",
          ...
          "  API **200ms→80ms**, payload **54%** 절감, **모바일에서도 무거운 3D 씬을 안정적으로 처리할 수 있도록** **LoD 자동 생성**으로 3D 모델 **90%** 경량화. Draw call **3,000→20(99%)**, 모델 로딩 **10초→2.5초**로 모바일 렌더링 병목을 해소했습니다.",
          ...
          "**기술 리더십**: 신규 입사자·FE/ML 인원을 **온보딩과 코드 리뷰**로 백엔드에 독립 기여할 수 있도록 이끌었습니다.",
    ...
    experience: [
      {
        ...
        description: [
          "**Ohouse AI Launch**: Completed global launch to **170 countries in 4 weeks**, accelerating a 6-week target. Built queue-based async workers with autoscaling, failure isolation, and reusable retry/state patterns to separate long-running AI jobs from user-facing APIs and handle unpredictable traffic safely.",
          ...
          "  API latency **200ms→80ms**, payload **54%** reduction, **LoD auto-generation** for **90%** model size reduction **to make heavy 3D scenes viable on mobile**. Draw calls **3,000→20 (99%)**, model loading **10s→2.5s**, resolving mobile rendering bottlenecks.",
