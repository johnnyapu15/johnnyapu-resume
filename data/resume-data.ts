...
export const resumeData: MultiLanguageResumeData = {
  ko: {
    ...
    summary:
      "대규모 백엔드 시스템 설계와 최적화에 강점을 지닌 소프트웨어 엔지니어입니다. 급격한 트래픽 증가 상황에서도 안정적인 서비스를 제공하기 위해, 장애 전파 방지 구조와 데이터 처리 효율화, API 성능 개선, 비동기 병렬 처리 등을 설계하고 구현해왔습니다. 자동화 파이프라인 구축을 통해 운영 효율을 27% 향상시키는 등 생산성 향상에 지속적으로 기여하며, 매 분기별 **Top Contributor로 선정**된 경험이 있습니다. 현재 Bucketplace에서 **시스템 디자인과 성능 개선, 그리고 AI 도구를 활용한 업무 효율화**를 주도하며 **백엔드 리드** 역할을 수행하고 있습니다.",
    experience: [
      {
